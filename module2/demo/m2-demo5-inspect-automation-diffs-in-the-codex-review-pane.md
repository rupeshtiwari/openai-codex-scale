...
Expect two modified files: `apps/api/src/utils/priority.ts` and `docs/triage-rubric.md`.
...
**Expected result.** Two files. `apps/api/src/utils/priority.ts` gains three lines mapping `sev1`,
...
1. Click `apps/api/src/utils/priority.ts`. The diff opens with the three added lines.
...
```text
 apps/api/src/utils/priority.ts | 4 ++++
