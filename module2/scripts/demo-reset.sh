...
$FMT box "Module 2 reset" "Return fixtures and working tree to the demo starting state"
...
UNTRACKED=$(git ls-files --others --exclude-standard apps docs automation | wc -l | tr -d ' ')
if [ "$TRACKED" -gt 0 ] || [ "$UNTRACKED" -gt 0 ]; then
  git checkout -- . 2>/dev/null
  git clean -fd apps docs automation >/dev/null 2>&1
...
for f in automation/sentry-fixtures/issues.json \
         automation/github-fixtures/commits.json \
