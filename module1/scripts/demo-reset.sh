...
$FMT box "Module 1 reset" "Return the repository to the demo starting state"
...
UNTRACKED=$(git ls-files --others --exclude-standard apps plans | wc -l | tr -d ' ')
if [ "$TRACKED" -gt 0 ] || [ "$UNTRACKED" -gt 0 ]; then
  git checkout -- . 2>/dev/null
  git clean -fd apps plans >/dev/null 2>&1
...
for plan in plans/refactor-execplan.md plans/migration-execplan.md; do
