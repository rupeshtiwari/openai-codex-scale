...
$FMT box "Module 1 demo up" "Start the SupportHub API and confirm it answers"
...
if lsof -ti:3000 >/dev/null 2>&1; then
  ...
  node apps/api/dist/src/server.js > "${ROOT}/preflight-logs/module1_server.log" 2>&1 &
...
if [ -n "$HEALTH" ]; then
  $FMT star "health" "$HEALTH"
  $FMT verdict pass "Module 1 environment is up."
  exit 0
fi
$FMT verdict fail "Service did not answer on port 3000. See preflight-logs/module1_server.log"
exit 1
