...
OUT="${ROOT}/preflight-logs/module1_demo_output.txt"
cd "$ROOT"
mkdir -p "$(dirname "$OUT")"
...
echo
echo "\$ grep -rl \"value === 'p0'\" apps/api/src"
grep -rl "value === 'p0'" apps/api/src
echo "  EXPECTED  three files"
echo
echo "\$ grep -rn \"from '.*utils/legacy\" apps/api/src apps/api/tests | wc -l"
grep -rn "from '.*utils/legacy" apps/api/src apps/api/tests | wc -l
echo "  EXPECTED  0 importers"
echo
echo "\$ wc -l apps/api/src/services/ticketService.ts"
...
echo
echo "\$ sed -n '/## Intended changes/,/## Behavior contracts/p' plans/refactor-execplan.md"
...
echo
echo "\$ find apps/legacy-ticket-api -type f -not -path '*/node_modules/*' | sort"
...
echo
echo "\$ grep -n __dirname apps/legacy-ticket-api/services/ticketService.js"
...
node -p "'  legacy: express ' + require('./apps/legacy-ticket-api/node_modules/express/package.json').version"
...
$FMT box "Module 1 output capture" "Deterministic evidence behind every demo claim"
$FMT star "written to" "preflight-logs/module1_demo_output.txt"
